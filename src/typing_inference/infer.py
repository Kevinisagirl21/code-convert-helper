"""Local type inference for code-convert-helper's v1 core subset.

This module never guesses a concrete type it isn't confident about. Where
inference can't resolve a type, it returns a :class:`~ir.schema.TypeHole`
...
def reset_hole_counter() -> None:
    """Reset the type-hole ID counter. Mainly useful for deterministic tests."""
...
def new_hole(known_info: list[str] | None = None) -> schema.TypeHole:
    """Public constructor for a fresh :class:`~ir.schema.TypeHole`.
...
def infer_return_type(
    ...
    * Returns disagree, or any can't be resolved -> a :class:`~ir.schema.TypeHole`
